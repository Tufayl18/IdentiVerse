...
// import UserProfileDisplay from "./user-profile-displayser-profile-display";
...
export default function createprofile() {
    ...
    useEffect(() => {
        const getUserInfo = async () => {
            const userInfo = (await getUserByAddress(wallets[0]?.address)) as any
            const username = (await getUsernameByAddress(wallets[0]?.address)) as any
    ...
    useEffect(() => {
        const fetchCountryCode = async () => {
            try {
                ...
                const data = response.data.country_code
                setCountryCode(data)
    ...
    const handleChange = (name: string, value: any) => {
        ...
        } else if (["x", "linkedin", "youtube"].includes(name)) {
            const pattern = urlPatterns[name]
            if (pattern) {
                const validUrl = validateUrl(value, pattern)
                if (!validUrl) {
        ...
        setError((prevErrors: any) => ({ ...prevErrors, [name]: error }))
    }

    const skillChange = (selected: any) => {
        ...
        >
            <div>
                <div className="md:text-4xl text-xl font-medium w-3/3 pb-3">
                    ...
                    <span className="text-sky-500">IdentiVerse</span>
                </div>
                <Toaster />
...
//b03a77ab46ecad08a6c7af35b27a7c32864fd7f9775a23fee93795d43a811637
